...
#define myvec vector < NewDesviacion >
#define myhash unordered_map <string , NewDesviacion >
...
struct NewDesviacion
{
    ...
    NewDesviacion(string b, float r,int n,float o) : book(b), desv(r), cardinalidad(n),opcional(o) {}
};
struct less_distance
{
    inline bool operator() (const NewDesviacion& struct1, const NewDesviacion& struct2)
    {
...
void calcular_desviacion(string user,string book)
{
    ...
    while(it_book_per_user!=it_m_user->second->end())
    {
        ...
        list_desv.insert({book,NewDesviacion(it_book_per_user->first,p.first,p.second,0.0)});
        it_book_per_user++;
    }
}
