...
myvec k_nn(int col,int k)
{
    ...
    for(int i =0;i<size_m;i++)
    {
        vec.push_back(MyStruct(i,distancia_euclidea(col,i),0.0,0.0)); 
    }
...
int main()
{
    ...
    cout<<"Correlacion de Pearson:\t"<<correlacion_pearson(nombres.find("Patrick C")->second,21)<<endl;
    ...
    //myvec knn=k_nn(nombres.find("brian")->second,3);

    myvec knn=k_nn(8,3);
    //calcular_influencia(nombres.find("brian")->second,knn);
    calcular_influencia(8,knn);
    ...
    //cout<<"proyeccion:\t"<<proyectado_knn(knn,libros.find("Forest Gump")->second)<<endl;
    cout<<"proyeccion:\t"<<proyectado_knn(knn,6)<<endl;
